...
bool CRecentlyAddedJob::UpdateVideo()
{
  ...
  if (!CSettings::GetInstance().GetBool(CSettings::SETTING_VIDEOLIBRARY_SHOWSEENINHOME))
  ...
  i = 0;
  if (CSettings::GetInstance().GetInt(CSettings::SETTING_VIDEOLIBRARY_TVSHOWSINHOME) == SETTINGS_VIDEODB_TVSHOW_HOME_EPISODES)
  {
  ...
  else
  {
    ...
    switch (CSettings::GetInstance().GetInt(CSettings::SETTING_VIDEOLIBRARY_TVSHOWSINHOME))
    {
